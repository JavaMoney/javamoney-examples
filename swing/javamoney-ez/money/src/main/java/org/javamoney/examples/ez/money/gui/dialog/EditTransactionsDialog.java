...
/**
   ...
   */
  public
  EditTransactionsDialog(ArrayList<RegisterTransaction> list)
  {
    super(675, 375);
  ...
  /**
   * This method shows a dialog for editing a group of transactions. This method
   * returns true if the dialog was accepted, otherwise false.
   *
  ...
  createDialogHeader()
  {
    ...
    return new DialogHeader(title, description, DIALOG_EDIT_TRANSACTIONS.getIcon());
  }
  ...
  createEditPanel()
  {
    ...
    {
      tabs.addTab(getSharedProperty("expenses"), createTypePanel(EXPENSES));
    }
  ...
  createPayeeChoosers()
  {
    ...
    for(int len = 0; len < getPayeeChoosers().length; ++len)
    {
      ...
      getPayeeChoosers()[len].getTextField().setDocument(new TextConstrainer(MAX_PAYEE_LENGTH));
  ...
  createTypePanel(int type)
  {
    ...
    panel.add(new SelectableComponentPanel(getCheckFields()[type]), 3, 0, 1, 1, 0, 0);
    panel.add(new SelectableComponentPanel(getDateLinks()[type]), 3, 1, 1, 1, 0, 0);
    panel.add(new SelectableComponentPanel(getPayeeChoosers()[type]), 1, 2, 1, 1, 100, 0);
    panel.add(new SelectableComponentPanel(getAmountFields()[type]), 3, 2, 1, 1, 0, 0);
    panel.add(new SelectableComponentPanel(getCategoryChoosers()[type]), 1, 3, 1, 1, 0, 0);
    panel.add(new SelectableComponentPanel(getNotesFields()[type]), 1, 4, 1, 1, 0, 0);
  ...
  doEditFor(int type)
  {
    ...
    Money amount = Money.of(UI_CURRENCY_SYMBOL.getCurrency(), Double.valueOf(0));
    Date date = new Date();
  ...
  private
  void
  organizeTransactions(ArrayList<RegisterTransaction> list)
  {
