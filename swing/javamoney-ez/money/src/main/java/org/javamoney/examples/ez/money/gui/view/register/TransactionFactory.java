 ...
 */
abstract class TransactionFactory extends Panel {
	/**
	 * 
	 */
	 ...
	 * 
	 * @param key
	 *            The type of transactions to create.
	 */
	...
	/**
	 * This method adds the specified transaction to the current account. It
	...
	protected final void clear() {
		...
		getForm().getField(DATE).setText(
				UI_DATE_FORMAT.format(getLastUsedDate()));
	 ...
	 * @param startEdit
	 *            Whether or not to start editing the transaction.
	 */
	 ...
	 *            true or false.
	 */
	protected final void enableForm(boolean value) {
		getForm().enableForm(value);

		if (getForm().getType() == TRANSFER) {
			// Initialize both chooser with the current account. At least one
			// will be
			// correct.
			if (isInEditMode() == false) {
				getForm().getPayFromChooser().setSelectedItem(
						getAccount().getIdentifier());
				getForm().getPayToChooser().setSelectedItem(
						getAccount().getIdentifier());
			}
	...
	private void applyChanges() {
		...
		if (transAdded != null) {
			if (isInEditMode() == true) {
				...
				setNewTransactionsArePending(getForm().getButton(PENDING)
						.isSelected());
			}
	...
	private void fillFormWith(Transaction trans) {
		...
		if (getForm().getType() == TRANSFER) {
			...
			if (isSplit(trans) == true) {
				...
				getForm().getPayToChooser()
						.setSelectedItem(trans.getCategory());
			}
		}
	}

	private Transaction getEditModeTransaction() {
		return itsEditModeTransaction;
	}

	private Date getLastUsedDate() {
	...
	private void setAmount(double value) {
		getForm().getField(AMOUNT).setText(
				UI_CURRENCY_FORMAT.format(Math.abs(value)));
	}

	private void setAmount(MonetaryAmount value) {
		setAmount(value.getNumber().doubleValue());
	}

	private void setEditModeTransaction(Transaction trans) {
		itsEditModeTransaction = trans;
	}

	private void setForm(Form panel) {
		itsForm = panel;
	}
	...
	private void setLastUsedDate(Date date) {
