...
final class QIFTransactionExtracter extends TransactionExtracter {
	 ...
	 */
	protected QIFTransactionExtracter() {
		setAccountBalance(Money.of(Double.valueOf(0),
	 ...
	 */
	protected AccountTypeKeys getAccountKey() {
	 ...
	 * transactions into, or null if one was not specified in the file.
	 * 
	 * @return The account's unique identifier or null if one was not specified
	 *         in the file.
	 */
	...
	/**
	 * This method returns true if an account for importing the transactions
	 * into has been specified from within the file, otherwise false.
	 * 
	 * @return true or false.
	 */
	protected boolean hasAccountForImport() {
	...
	protected Transaction next(BufferedReader stream) throws Exception {
		...
		while ((line = stream.readLine()) != null) {
			...
			if (key == END_OF_ENTRY) {
				...
				// If the category isn't already formatted to be a split, then
				// reset it.
				if (isSplit(category) == false) {
					category = "";
				}

				category += ITEM_SEPARATOR + extractCategory(line)
						+ AMOUNT_SEPARATOR;
		...
		if (line != null) {
			...
			trans = new Transaction(number, date, payee, Money.of(amount,
					UI_CURRENCY_SYMBOL.getCurrency()), category, notes);
	...
	private void extractAccountInfo(BufferedReader stream) throws Exception {
		String line = "";
		char key;
		...
		while ((line = stream.readLine()) != null) {
			...
			if (key == END_OF_ENTRY) {
				...
				setAccountBalance(Money.of(extractAmount(line),
						UI_CURRENCY_SYMBOL.getCurrency()));
	...
	private static AccountTypeKeys extractAccountKey(String line) {
	...
	private void setAccountBalance(Money balance) {
		itsAccountBalance = balance;
	}

	private void setAccountKey(AccountTypeKeys key) {
		itsAccountKey = key;
	}

	private void setAccountUID(String uid) {
		itsAccountUID = uid;
	}
