 ...
 */
public final class DataIOManager {
	/**
	 ...
	 * 
	 * @param file
	 *            The file to have the data read from.
	 * @param showError
	 *            Whether or not to show an error message on failure.
	 * @param decrypt
	 *            Whether or not the data needs to be decrypted.
	 * 
	 * @return true or false.
	 */
	protected static boolean read(File file, boolean showError, boolean decrypt) {
		...
		try {
			...
			if (showError == true) {
				error(getProperty("read.title"),
						getProperty("read.description") + "<br><br><b>" + file
								+ "</b>");
			}
		}
	 ...
	 * 
	 * @param file
	 *            The file to have the data written to.
	 * @param showError
	 *            Whether or not to show an error message on failure.
	 * @param encrypt
	 *            Whether or not the data needs to be encrypted.
	 * 
	 * @return true or false.
	 */
	protected static boolean write(File file, boolean showError, boolean encrypt) {
		boolean result = false;
		...
		try {
			...
			if (showError == true) {
				error(getProperty("write.title"),
						getProperty("write.description") + "<br><br><b>" + file
								+ "</b>");
			}
		}
	...
	private static PrintStream createPrintStream(File file, boolean encrypt)
			throws Exception {
	...
	private static BufferedReader createReader(File file, boolean decrypt)
			throws Exception {
	...
	private static void readAccounts(BufferedReader stream) throws Exception {
		...
		while ((line = stream.readLine()) != null) {
			...
			if (key == END_OF_COLLECTION) {
				...
				account.setBalance(Money.of(US_DOLLAR.parse(line),
	...
	private static void readCategoryCollection(CategoryCollection collection,
			BufferedReader stream) throws Exception {
	...
	private static void readPayees(BufferedReader stream) throws Exception {
	...
	private static void readReminders(BufferedReader stream) throws Exception {
	...
	private static void readTransactions(BufferedReader stream)
			throws Exception {
		Account account = new Account(DEPOSIT, "");
		String amount = "";
		String category = "";
		...
		while ((line = stream.readLine()) != null) {
			...
			if (key == END_OF_COLLECTION) {
				...
				Transaction trans = new Transaction(number,
						DATE_FORMAT.parse(date), payee, Money.of(
								US_DOLLAR.parse(amount),
								UI_CURRENCY_SYMBOL.getCurrency()), category,
						notes);
	...
	private static void writeAccounts(PrintStream stream) {
		...
		for (DataElement element : getAccounts().getCollection()) {
			...
			stream.println(ACCOUNT_BALANCE
					+ US_DOLLAR.format(account.getBalance().getNumber()
							.doubleValue(), false));
	...
	private static void writeCategory(Category category, PrintStream stream) {
		stream.println(UID + category.getIdentifier());
		stream.println(CATEGORY_GROUP + category.getGroupName());

		if (category.isBudgeted() == true) {
			stream.println(CATEGORY_BUDGET
					+ String.valueOf(category.getBudget()));
			stream.println(CATEGORY_DATE
					+ DATE_FORMAT.format(category.getRolloverStartDate()));
			stream.println(CATEGORY_ROLLOVER
					+ String.valueOf(category.hasRolloverBudget()));
		}
	...
	private static void writeCategoryCollection(CategoryCollection collection,
			PrintStream stream) {
	...
	private static void writePayess(PrintStream stream) {
	...
	private static void writeReminders(PrintStream stream) {
		...
		for (DataElement element : getReminders().getCollection()) {
			...
			stream.println(REMINDER_DUE_BY
					+ DATE_FORMAT.format(reminder.getDueBy()));
			stream.println(REMINDER_DAYS_TO_ALARM
					+ String.valueOf(reminder.getDaysToAlarm()));
			stream.println(REMINDER_COMPLETE
					+ String.valueOf(reminder.isComplete()));
	...
	private static void writeTransactions(PrintStream stream) {
		...
		for (DataElement element : getAccounts().getCollection()) {
			...
			for (Transaction trans : account.getTransactions()) {
				...
				stream.println(TRANS_AMOUNT
						+ USD
						+ US_DOLLAR.format(trans.getAmount().getNumber()
								.doubleValue(), false));
				...
				stream.println(TRANS_RECONCILED
						+ String.valueOf(trans.isReconciled()));
				stream.println(END_OF_ENTRY);
			}
		}
	...
	private static final CurrencyFormat US_DOLLAR = CurrencyFormatKeys.US_DOLLAR
			.getFormat();
