 ...
 */
public final class BudgetReport extends Report {
	/**
	 ...
	 */
	public static BudgetReport createReport(Date start, Date end,
			TotalFilter filter) {
	 ...
	 */
	public double getRolloverTotal() {
	...
	private void addBudgets() {
		...
		for (DataElement element : getExpenses().getCollection()) {
			...
			if (category.isBudgeted() == true) {
				getBudgets().add(
						new Budget(CategoryTotalTypeKeys.EXPENSE, category));
			}
		}
		...
		for (DataElement element : getIncome().getCollection()) {
			...
			if (category.isBudgeted() == true) {
				getBudgets().add(
						new Budget(CategoryTotalTypeKeys.INCOME, category));
			}
		}
	}

	private void addCategory(Account account, Transaction trans) {
		...
		if (budget != null) {
			if (getFilter().allowsCategory(trans) == true) {
				if (isInRange(trans, getStartDate(), getEndDate()) == true) {
					TransactionDetail detail = new TransactionDetail(trans,
							account);

					budget.setChange(budget.getChange()
							+ Math.abs(trans.getAmount().getNumber()
									.doubleValue()));
					...
					if (trans.getDate().before(getStartDate()) == true) {
						if (isOnOrAfter(trans, budget.getRolloverStartDate()) == true) {
							budget.setStartingBalance(budget
									.getStartingBalance()
									+ trans.getAmount().getNumber()
											.doubleValue());
						}
					}
				}
	...
	private void addSplit(Account account, Transaction trans) {
		Split split = new Split(trans);

		for (int index = 0; index < split.size(); ++index) {
			trans = trans.clone();
			trans.setAmount(Money.of(split.getAmount(index),
	...
	private void calculateTotals() {
		...
		for (Budget budget : getBudgets()) {
			...
			if (budget.hasRolloverBalance() == true) {
				int startSpan = getMonthSpan(budget.getRolloverStartDate(),
						getStartDate());
				double startingBalance = budget.getAmount() * startSpan;

				budget.setStartingBalance(budget.getStartingBalance()
						+ startingBalance);
			}
	...
	private Budget getBudgetAt(String category) {
	...
	private void setBalanceTotal(double amount) {
		itsBalanceTotal = amount;
	}

	private void setBudgets(Collection<Budget> collection) {
		itsBudgets = collection;
	}

	private void setBudgetTotal(int amount) {
		itsBudgetTotal = amount;
	}

	private void setChangeTotal(double amount) {
		itsChangeTotal = amount;
	}

	private void setRolloverTotal(double amount) {
